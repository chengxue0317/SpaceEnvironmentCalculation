...
def main(t,alt,outpath):
   # try:
    ...
    map.drawcoastlines(linewidth=0.1,color='0.2')
    # map.drawmeridians(np.arange(-180, 180, 30))
    # map.drawparallels(np.arange(-90, 90, 30))
    Lon, Lat = np.meshgrid(lon, lat)
    X, Y = map(Lon, Lat)
    lvls = np.linspace(B_.min(), B_.max(), 500)
    C = plt.contourf( X, Y , B_, levels=lvls, alpha=1, cmap=plt.cm.rainbow,extend='both')  # ,levels=lvls,cmap=plt.cm.rainbow)#
    ...
    fig.subplots_adjust(bottom=0.5,left=0.02, right=0.98)
    norm = mpl.colors.Normalize(vmin=B_.min(), vmax=B_.max())
    clb=fig.colorbar(
        mpl.cm.ScalarMappable(norm=norm, cmap="rainbow"),
        cax=ax,
        orientation="horizontal",
    )
    clb.set_label(label='Magnetic field/(nT)',loc='right',color='white')
    # clb.ax.set_title(label='Density (×$\mathregular{10^5}$)',color='white')
    clb.ax.tick_params(labelcolor='white')
    clb.ax.tick_params(color='white')
    # clb.ax.tick_params(labelsize=8)
    # print(name)
    name1=t.replace(' ', '_').replace(':', '-') + '_' + str(alt) +'km_colorbar.png'
    plt.savefig(os.path.join(outpath, name1), format='png',dpi=300, pad_inches=0.0)
    # print(name1)
    out={'fig':name,'bar':name1}
    print(out)
   # except:
   #     print(False)
