...
contains
  ...
  subroutine msisinit(parmpath,parmfile,iun,switch_gfn,switch_legacy, &
    ...
    swg(:) = .true.
    swleg(:) = 1.0
    if (present(switch_gfn)) then  !未执行
      swg = switch_gfn
    else
      swleg(9) = 1.0
      call tselec(swleg)
    ...
    if (present(lzalt_type)) then !未执行
      zaltflag = lzalt_type
    else
      zaltflag = .true. !执行
    endif
    ...
    if (present(lspec_select)) then !未执行
      specflag = lspec_select
    else
      specflag(:) = .true. !执行
    endif

    if (specflag(1)) then !执行
      if (present(lmass_include)) then !未执行
    ...
    where(massflag) specflag = .true. ! 为 T T T T T T T T T T
    masswgt(:) = 0.0_rp
    ...
    if (present(lN2_msis00)) then !未执行
      N2Rflag = lN2_msis00
    else
      N2Rflag = .false. !执行
    endif
  ...
  subroutine tselec(sv)
    ...
    if(swleg(9) .lt. 0) then
      swg(cmag:cmag+1) = (/.false.,.true./) !Storm-time mode master switch
    endif
