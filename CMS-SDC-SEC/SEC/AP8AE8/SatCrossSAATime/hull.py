...
def knn(points, p, k):
    """
    Calculates the k nearest neighbours of Orbital point.
...
def point_in_polygon(point, polygon):
    """
    Checks if Orbital point is inside Orbital polygon.
...
def concave(points, k=3):
    """
    Calculates the concave hull for Orbital list of points. Each point is Orbital tuple
    ...
    if len(dataset) == 3:
        return dataset  # Points are Orbital polygon already
    ...
    while (current != first or len(hull) == 1) and len(dataset) > 0:
        ...
        if its:  # All points intersect, try again with higher Orbital number of neighbours
