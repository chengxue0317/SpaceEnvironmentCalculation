Program ap8ae8
    ...
    real(kind = 4), dimension(NTIME_MAX) :: lat, lon, alt
    ...
    do i = 1, lineCounts
        read(100, *) temp_real, BB0(i), Lm(i), lat(i), lon(i), alt(i), date(i), time(i)
    end do
    Lm = abs(Lm)
    close(100)
    ...
    case(1, 2)
        nene = 5
        energy(1, 1:5) = (/0.15, 0.35, 0.65, 1.2, 2.0/)
        energy(2, 1:5) = (/0.35, 0.65, 1.2, 2.0, 5.7/)
    case(3, 4)
        nene = 6
        energy(1, 1:6) = (/3, 5, 10, 26, 40, 100/)
        energy(2, 1:6) = (/5, 10, 26, 40, 100, 300/)
    end select
    ...
    do i = 1, lineCounts
        write(100, "(3(g0,:,', '),A10,1x,A8,:,', ',*(g0,:,', '))") lat(i), lon(i), alt(i), date(i), time(i), flux(i, 1:nene)
    end do
    close(100)
