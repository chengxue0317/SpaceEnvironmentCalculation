...
def get_DB(S1_strength,S2_strength):

    all_DB_S1,all_DB_S2 = [],[]
    s1_st = su_data([float(i) for i in S1_strength], 60)
    s1_st_new = [float(j / 60) for j in s1_st]
    for h in range(len(s1_st_new)):
        if h !=0 :
            h_new_s1 = s1_st_new[h]-s1_st_new[h-1]
            if h_new_s1 >=0 :
    ...
    s2_st = su_data([float(i) for i in S2_strength], 60)
    s2_st_new = [float(j / 60) for j in s2_st]
    for h in range(len(s2_st_new)):
        if h != 0:
            h_new_s2 = s2_st_new[h] - s2_st_new[h - 1]
            if h_new_s2 >= 0:
...
def read_data(file_path, params_system, PRN, P1_channel, P2_channel):
    ...
    return all_time, S1_S4_value_average,S2_S4_value_average,S1_S4_value_square, S2_S4_value_square, S1_strength, S2_strength
...
def main(file_path, params_system, PRN, P1_channel, P2_channel, stat_time, end_time):
    ...
    all_DB_S1, all_DB_S2 = get_DB(S1_strength, S2_strength)
